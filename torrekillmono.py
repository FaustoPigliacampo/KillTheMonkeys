...
# Funciones
def mono_destruido(monos, balas_simples):
    monos.eliminar()
...
def morir(mono, torreta):
    global fin_del_juego, monos, agregar_mono
    agregar_mono.terminar()
    torreta.eliminar()
    texto_muerte = pilas.actores.Texto("Fin del Juego")
    texto_muerte.color = pilas.colores.amarillo
    texto_muerte.escala = 2
    texto_muerte.y = -60
    pilas.avisar("Conseguiste %d puntos" %(puntos.obtener()))
    mono.eliminar()

pilas.colisiones.agregar(monos, torreta, morir)
...
agregar_mono = pilas.tareas.agregar(1, crear_mono)
